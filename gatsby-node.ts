...
exports.onCreateWebpackConfig = ({ stage, actions, getConfig }) => {
  ...
  config.plugins.push(FontaineTransform.webpack(fontaineOptions))
...
exports.createPages = async ({ actions, graphql }) => {
  ...
  data.allState.edges.forEach(
    (state: { node: { meta: { slug: any }; id: any; childrenCounty: any[]; childrenProblem: any[] } }) => {
      ...
      state.node.childrenProblem.forEach((problem) => {
        ...
        actions.createPage({
          path: pagePath,
          component: path.resolve(`./src/templates/mapPage.tsx`),
          ownerNodeId: `${problem.id}`,
          context: {
            id: problem.id,
            title: problem.title,
            slug: problem.meta.slug,
            state: state.node.meta.slug
          }
        })
      })
    }
